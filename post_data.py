...
# 0220.3/04. 20:15, review by Queenie
...
def query_and_insert(item, start_time=s, end_time=e):
    ...
    # sqlalchemyobj = SQLAlchemy(thread, metadatafrom_sqlacodegen)
    query_result = sqlalchemyobject.session.query(table_name).filter(sql_where_statement).all()
